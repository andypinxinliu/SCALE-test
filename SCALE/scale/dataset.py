...
# 如果有本地该文件夹名可以直接访问到文件并return load_file(root)
def load_files(root):
    """
...
# call method在SCALE的main里，详见SCALE文件
def load_dataset(
        data_list, 
        batch_categories=None, 
        join='inner', 
    ...
    # 建立一个scdata的对象，并保存在scdata中
    scdata = SingleCellDataset(adata) # Wrap AnnData into Pytorch Dataset
    
    # 使用pytorch自带的dataloader来train上面的scdata文件
...
#     batch_sampler = BatchSampler(batch_size, adata.obs['batch'], drop_last=False)
    ...
    #同时建立testloader，和trainloader的区别在drop_last和shuffle
